...
// 중심 좌표를 추적하는 컴포넌트
const MapCenterUpdater = ({ setPosition }) => {
  useMapEvents({
    moveend(event) {
      const center = event.target.getCenter();
      setPosition([center.lat, center.lng]);
      console.log("지도 중심 좌표:", center); // 콘솔에 표시
    },
  });
  return null;
};
...
const Home = () => {
  ...
  const initialPosition = [35.120696, 129.0411816];
  ...
  return (
      <div className="mainSection">
        ...
        {/* Leaflet 지도 추가 */}
        <MapContainer center={position} zoom={14} style={{ height: "100vh", width: "calc(100vw - 220px)",position:"absolute",left:"220px" }}>
          <TileLayer
          ...
          {/* 중심 좌표 추적 컴포넌트 */}
          <MapCenterUpdater setPosition={setPosition} />
